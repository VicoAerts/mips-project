...
###############################################################################################
main_game_loop:
move $t7,$a0 #verplaats tijdellijk a0 naar t0
...
input_verwerken:
	beq $s0,122,omhoog 		#wanneer we z krijgen beweeg omhoog 	
	beq $s0,115,omlaag		#wanneer we s krijgen beweeeg omlaag
	beq $s0,113,links		#wanneer we q krijgen beweeeg naar links
	beq $s0,100,rechts		#wanneer we d krijgen beweeg naar rechts
	beq $s0,120,einde_game		#beindig spel
		
error:
	la $a0,error_msg	#laad de error message in a0
	li $v0,4		#print str
	syscall
	j main_game_loop			#jump terug

omhoog:
	move $a0,$t7 			#plaats t0 terug in a0 / a terug oorspronkelijke waarde
	subi $a2,$a0,1
	move $a3,$a1
	j change_player			
omlaag:
